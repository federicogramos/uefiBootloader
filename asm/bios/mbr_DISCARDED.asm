...
ext_supported:
	...
	mov bx, 6117	;; Start offset.
	mov cx, 0x8000	;; Destination.
...
load_nextsector:
	call readsector		;; Each loop loads 512 bytes.
	dec ax
	cmp ax, 0
	jnz load_nextsector
...
readsector:
	push ax
	xor ax, ax	; We don't need to load from sectors > 32-bit
	push dx
	push si
	push di

read_it:
	push ax			; Save the sector number
	push bx
	mov di, sp			; remember parameter block end

	push ax			; [C] sector number high 32bit
	push bx			; [8] sector number low 32bit
...
read_ok:
	add bx, 1			; increment next sector with carry
	adc ax, 0
